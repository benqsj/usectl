...
const SLIDERS: SliderSpec[] = [
  { key: "trailThicknessPx", label: "Trail thickness", min: 1, max: 12, step: 0.1, unit: "px" },
  { key: "trailFadeMs", label: "Trail fade", min: 100, max: 1200, step: 10, unit: "ms" },
  { key: "trailGlowPx", label: "Trail glow", min: 0, max: 60, step: 1, unit: "px" },
  { key: "trailSmoothing", label: "Trail smoothing", min: 0, max: 0.9, step: 0.02 },
...
export function LabLinesClient() {
  ...
  // The dummy box's crosses (and its trail region) are simply always on, so both can be judged
  // against the lines by drawing inside and outside the box.
  useEffect(() => {
  ...
  const copyRow = useCallback(() => {
    const row = [
      `| \`trailThicknessPx\` | ${params.trailThicknessPx} |`,
      `| \`trailFadeMs\` | ${params.trailFadeMs} |`,
      `| \`trailGlowPx\` | ${params.trailGlowPx} |`,
      `| \`trailSmoothing\` | ${params.trailSmoothing} |`,
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-hidden bg-background font-mono text-xs text-white/70">
      {/* The real grid — the CSS fallback that paints first, the static canvas that takes over, and
          the pointer trail on top of both. */}
      <div className="pointer-events-none absolute inset-0">
        <CssGridLines />
        <GridCanvas params={params} />
        <GridTrail params={params} />
      </div>
      ...
      {/* Resizable stand-in for a section's content: drag it, resize it from the corner, and draw
          inside the crosses' rectangle to see the trail follow the pointer. */}
      <div
        ref={boxRef}
        data-lab-ui
      ...
      >
        <span className="absolute top-1 left-2 text-white/40">
          drag me · resize from the corner · draw inside the crosses for the trail
        </span>
        <div
      ...
      <div data-lab-ui className="absolute top-[120px] right-6 max-h-[calc(100vh-140px)] w-[320px] overflow-y-auto rounded border border-white/15 bg-black/70 p-4 backdrop-blur-sm">
        <div className="mb-3 text-[11px] tracking-wide text-white/40 uppercase">grid trail lab</div>
