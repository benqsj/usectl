...
export interface GridDrawState {
  ...
  mouseX: number;
  mouseY: number;
  /** 0..1 — the whole deformation fades out when the pointer leaves */
  active: number;
  /** seconds since start, only read by the ripple variant */
  time: number;
  radius: number;
  strength: number;
  /** +1 = pull, -1 = push */
  sign: number;
  /** 0 = pull/push, 1 = ripple */
  variant: number;
  rippleFreq: number;
  rippleSpeed: number;
  metrics: GridMetrics;
...
export function createGridRenderer(canvas: HTMLCanvasElement): GridRenderer | null {
  ...
  return {
    ...
    draw(s) {
      ...
      gl.uniform1f(u.u_active, s.active);
      gl.uniform1f(u.u_time, s.time);
      gl.uniform1f(u.u_radius, Math.max(1, s.radius));
      gl.uniform1f(u.u_strength, s.strength);
      gl.uniform1f(u.u_sign, s.sign);
      gl.uniform1i(u.u_variant, s.variant);
      gl.uniform1f(u.u_rippleFreq, s.rippleFreq);
      gl.uniform1f(u.u_rippleSpeed, s.rippleSpeed);
