...
//
// The whole layer is ONE fullscreen pass: for each pixel, displace its coordinate around the
// cursor, then ask "is there a grid line (or a cross mark) at the displaced coordinate?". Because
// the lookup happens at the DISPLACED coordinate, the lines bend and the marks bend with them —
// there is nothing per-line to animate, and the rest state costs the same as the moving one.
//
// All maths is in CSS px with y measured from the top of the layer, which is exactly the coordinate
// system BackgroundLines.tsx's CSS lays the same grid out in (see lib/grid.ts). The rest state has
// to match that CSS pixel for pixel, so coverage is computed as a true box filter over the pixel's
// own footprint rather than a smoothstep — that is what a browser does when it rasterises a 2px div
// at a fractional position, and anything softer reads as a blurrier grid.
...
uniform float u_dpr;
uniform vec2  u_mouse;             // CSS px, y from the top
uniform float u_active;            // 0..1, eases in/out as the pointer enters/leaves
uniform float u_time;              // seconds, ripple only

uniform float u_radius;
uniform float u_strength;
uniform float u_sign;              // +1 = pull (towards the cursor), -1 = push
uniform int   u_variant;           // 0 = pull/push, 1 = ripple
...
void main() {
  ...
  vec2  d = p - u_mouse;
  float r = length(d);
  float fall = exp(-pow(r / u_radius, 2.0)) * u_active;
  vec2  dir = r > 0.0001 ? d / r : vec2(0.0);
  float amount = u_variant == 1
    ? u_strength * fall * sin(r * u_rippleFreq - u_time * u_rippleSpeed)
    : u_strength * fall * u_sign;
  vec2 q = p + dir * amount;

  // The pixel's footprint measured in the DISPLACED space: where the grid is stretched this
  // shrinks (crisper), where it is squeezed it grows (softer) — which is what keeps a bent line
  // looking like the same 2px line and not like an aliased staircase. At rest it is exactly
  // 1 device px, so the rest state falls back to the plain box filter.
  float hx = max(fwidth(q.x), 1e-4) * 0.5;
  float hy = max(fwidth(q.y), 1e-4) * 0.5;
  ...
  bool inHeader = q.y < u_header;
  bool underHeader = !inHeader && q.y < u_firstRowY;

  float colAlpha = 0.0;
  float rowAlpha = 0.0;

  if (!inHeader) {
    float ci = floor((q.x - u_inset) / u_colPitch + 0.5);   // 0-based column index
    if (ci >= 0.0 && ci < NUM_COLUMNS) {
      float cx = u_inset + ci * u_colPitch;
      float c = cov(q.x, cx - u_halfThickness, cx + u_halfThickness, hx);
      if (underHeader) {
        ...
        c *= cov(q.x, u_inset, u_containerRight, hx);
      }
    ...
    if (!underHeader) {
      ...
      float base = floor((q.y - u_header) / u_rowPitch);
      float c = 0.0;
      for (int k = 0; k <= 1; k++) {
        float j = base + float(k);
        if (j < 1.0) continue;
        float ry = u_header + j * u_rowPitch;
        c = max(c, cov(q.y, ry, ry + u_halfThickness * 2.0, hy));
      }
  ...
  // grid, and they bend with it because they are measured in the same displaced coordinate.
  for (int i = 0; i < 16; i++) {
    if (i >= u_markCount) break;
    vec4 m = u_marks[i];
    if (m.w <= 0.0) continue;
    float t = u_markHalfThickness;
    float horizontal = cov(q.y, m.y - t, m.y + t, hy) * cov(q.x, m.x - m.z, m.x + m.z, hx);
    float vertical   = cov(q.x, m.x - t, m.x + t, hx) * cov(q.y, m.y - m.z, m.y + m.z, hy);
