# Variant C · Exploded Instrument (Anime.js-inspired) — design reference
...
- `VariantC.dc.html` — the artboard (a Design Component: an `<x-dc>`
  template + a small logic class). The values to replicate live in its
  inline `style="…"` attributes and the `<helmet><style>` block.
- `ServerBridge.dc.html` — referenced resource
- `ServerSpin.dc.html` — referenced resource
...
`assets/` — 27 in this export — and the exported files refer to them there. A
...
Serve the folder (e.g. `python3 -m http.server`) and open `VariantC.dc.html`;
